...
module.exports = {
  ...
  get_input_addresses2: function(original_tx, original_txid, input, vout, cb) {
    var addresses = [];
    //console.log("get_input_addresses2() orig_txid=%s",original_txid);
    if (input.coinbase) {
      var amount = 0;
      module.exports.syncLoop(vout.length, function (loop) {
        var i = loop.iteration();
      ...
      if (original_txid == input.txid) {
        //console.log("same txid");
        var tx = original_tx;
        if (tx) {
          module.exports.syncLoop(tx.vout.length, function (loop) {
            var i = loop.iteration();
        ...
        //console.log("different orig=%s txid=%s",original_txid,input.txid);
        module.exports.get_rawtransaction(input.txid, function(tx){
        if (tx) {
          module.exports.syncLoop(tx.vout.length, function (loop) {
            var i = loop.iteration();
  ...
  prepare_vin: function(tx, cb) {
    ...
    var prev_txid;
    var prev_tx;
    //console.log("prepare_vin() for %d loop",tx.vin.length);
    module.exports.syncLoop(tx.vin.length, function (loop) {
      var i = loop.iteration();
      if (tx.vin[i].txid) {
        if (tx.vin[i].txid == prev_txid) {
          module.exports.get_input_addresses2(prev_tx, prev_txid, tx.vin[i], tx.vout, function(addresses){
          ...
          prev_txid = tx.vin[i].txid;
          module.exports.get_rawtransaction(tx.vin[i].txid, function(original_tx){
            if (original_tx) {
              prev_tx = original_tx;
              //console.log("calling get_input_address2(%s)",tx.vin[i].txid);
              module.exports.get_input_addresses2(original_tx, tx.vin[i].txid, tx.vin[i], tx.vout, function(addresses){
                if (addresses && addresses.length) {
                  //console.log('vin');
                  module.exports.is_unique(arr_vin, addresses[0].hash, function(unique, index) {
                    if (unique == true) {
        ...
        //console.log("calling get_input_address()");
